  ...
  const addRecorded = () => {
    for (const w of deal.workstreams || []) {
      ...
      for (const fnd of w.findings || []) {
        ...
        add(key, recordedSeverity(fnd.severity, 'condition'), text, `Recorded by the ${LANE_LABEL_FOR[key] || key} workstream.`, 'recorded');
      }
    }
  };
