...
// MP3 at a constant bitrate. Opus in WebM is smaller and is what an open-source Chromium
// build would need, but Azure returns it as a live stream with no duration in the header,
// so the player cannot show progress or seek within a scene. Every browser this will
// actually be opened in plays MP3.
const FORMAT = 'audio-24khz-96kbitrate-mono-mp3';
const FORMAT_KBPS = 96;
const EXT = 'mp3';
...
async function synthesise(auth, text, outPath) {
...
async function main() {
  ...
  for (const scene of manifest.scenes) {
    const file = `${scene.id}.${EXT}`;
    const abs = path.join(AUDIO, file);
    const size = await stat(abs).then((st) => st.size).catch(() => 0);
    const seconds = (bytes) => Math.round((bytes * 8) / (FORMAT_KBPS * 1000));

    if (size > 0 && !FORCE) {
      // Re-capturing rewrites the manifest, so restate the duration rather than
      // leaving the player without one.
      scene.audio = `audio/${file}`;
      scene.seconds = seconds(size);
      kept++;
      continue;
    }
    const bytes = await synthesise(auth, scene.say, abs);
    scene.audio = `audio/${file}`;
    scene.seconds = Math.round((bytes * 8) / (FORMAT_KBPS * 1000));
    made++;
