...
| 3 | **Diligence** (1:30) | Open a deal → **Overview**: the **decision cockpit** (verdict + top blockers + “what changed since last check”), then **Workspace** for the **RYG diligence workbench**. Hit **💬 Ask** *inside the deal*, ask a lane question, and **Apply ▸** the assistant's proposed next step — it writes an attributed entry to the **Activity** trail. | Decision cockpit; diligence workbench; approve-to-apply + audit trail |
| 4 | **Decide** (1:30) | Ask for the **LBO case (IRR / MOIC)**; open the **IC memo** and **IC voting precedents**. Tick **2–4 deals** on Deals Overview and open the **side-by-side compare** grid (Copy table). | Auto-drafted decision artifacts; IC governance; side-by-side compare |
...
| **Assistant approve-to-apply + audit trail** | "Can the AI change things?" — governance & provenance | The assistant **proposes** next steps but **never acts on its own**; a human clicks **Apply**, and every applied change writes a fully-attributed entry (who approved, when, via assistant) to the deal's **Activity** trail |
| **Decision cockpit + IC readiness** | Faster, defensible IC prep | Verdict + top blockers + “what changed since last check” + one-click resolve, all from real gating facts — not a progress bar |
