...
| **4** | **Connector governance, and Work IQ** · 2 min | **Settings ⚙ → Data sources**. Show that no source reports connected without a real round trip. Open **Helvetia Diagnostics → Workstreams → Files, chats & email on this deal** to show Teams, SharePoint, mail and durable shared conclusions together. Explain delegated Graph reads, read-only app access and the Copilot MCP surface. | Work IQ is demonstrated in the deal where it is used, with caller identity and source provenance intact. |
